...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div ref={ref} className={cn("sticky inset-x-0 top-2 z-40 w-full", className)}>
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl xl:max-w-[1600px] flex-row items-center justify-between self-start rounded-lg bg-transparent px-4 py-2 lg:flex",
        visible && "bg-white/80",
        className
      )}
    >
    ...
    >
      {items.map((item, idx) => (
        <Link
          key={`link-${idx}`}
          href={item.link}
          className="relative px-4 py-2 text-neutral-600 text-xl font-semibold"
          onClick={onItemClick}
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-lg bg-blue-400"
            />
          )}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white/90 backdrop-blur-md px-4 py-8 shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset]",
            className
          )}
        >
          {children}
...
export const MobileNavToggle = ({ isOpen, onClick }: { isOpen: boolean; onClick: () => void }) =>
  isOpen ? <IconX className="text-black" onClick={onClick} /> : <IconMenu2 className="text-black" onClick={onClick} />;
...
export const NavbarLogo = () => {
  return (
    <Link
      ...
      aria-label="Gama Color – Strona główna"
      prefetch
    >
      <Image src={logoGama} alt="Gama Color logo" width={150} height={150} />
    </Link>
  );
};
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md bg-yellow-400 button bg-yellow-400 text-black text-sm font-bold relative cursor-pointer hover:-translate-y-0.5 hover:bg-yellow-500 transition duration-200 inline-block text-center";
  const variantStyles = {
    primary:
